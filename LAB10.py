...
from keras.layers import Conv2D, MaxPool2D,GlobalAveragePooling2D,Dense,Input,Reshape,MaxPooling2D,UpSampling2D,BatchNormalization,GaussianNoise, Dropout
...
#     save_dir = "./saved_models"
#     model_json = autoencoder.to_json()
#     with open(os.path.join(save_dir, "own_autoencoder.json"), "w") as json_file:
#         json_file.write(model_json)

#     autoencoder.save_weights(os.path.join(save_dir, "own_autoencoder.h5"))

#zadanie 10.3
# def own_autoencoder(X_train):
#     act_func = 'relu'
#     encoder_layers = [GaussianNoise(0.1),
#                       Conv2D(32,(3,3),padding='same',activation=act_func),
#                       MaxPooling2D(2,2),
#                       Dropout(0.25),
#                       Conv2D(64,(3,3),padding='same',activation=act_func),
...
#     decoder_layers = [Conv2D(128,(3,3),padding='same',activation=act_func),
#                       UpSampling2D((2,2)),
#                       BatchNormalization(),
#                       Conv2D(64,(3,3),padding='same',activation=act_func),
#                       UpSampling2D((2,2)),
...
def own_autoencoder(X_train):
    ...
    encoded = tensor
    encoder = Model(inputs=autoencoder_input, outputs=encoded)
    for layer in decoder_layers:
        tensor = layer(tensor)
    ...
    autoencoder.compile(optimizer=Adam(learning_rate), loss='mean_squared_error')
    autoencoder.fit(x=X_train, y=X_train, epochs=20, batch_size=256)
    encoded_input = Input(shape=encoded.shape[1:])
    tensor = encoded_input
    for layer in decoder_layers:
        tensor = layer(tensor)
    
    decoder = Model(inputs=encoded_input, outputs=tensor)
    save_dir = "./saved_models"
    autoencoder_json = autoencoder.to_json()
    with open(os.path.join(save_dir, "own_autoencoder_4.json"), "w") as json_file:
        json_file.write(autoencoder_json)
    autoencoder.save_weights(os.path.join(save_dir, "own_autoencoder_4_weights.h5"))
    
    encoder_json = encoder.to_json()
    with open(os.path.join(save_dir, "encoder_4.json"), "w") as json_file:
        json_file.write(encoder_json)
    encoder.save_weights(os.path.join(save_dir, "encoder_4_weights.h5"))
    
    decoder_json = decoder.to_json()
    with open(os.path.join(save_dir, "decoder_4.json"), "w") as json_file:
        json_file.write(decoder_json)
    decoder.save_weights(os.path.join(save_dir, "decoder_4_weights.h5"))
...
if __name__ == '__main__':
    ...
    #zadanie 10.2
    # own_autoencoder(X_train_scaled)
    # own_autoencoder = load_model('./saved_models/own_autoencoder.json','./saved_models/own_autoencoder.h5')
    # test_photos = X_test[10:20,...].copy()
    # mask = np.random.randn(*test_photos.shape)
    # noisy_test_photos = add_salt_and_pepper_noise(test_photos)
    # cleaned_images = own_autoencoder.predict(noisy_test_photos)*255
    # show_pictures(test_photos)
    # show_pictures(noisy_test_photos)
    # show_pictures(cleaned_images)
